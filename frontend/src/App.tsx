...
function App() {
	...
	useEffect(() => {
		...
		const loadJob = async () => {
			setPolling(true);
			try {
				const nextJob = await apiRequest<JobResponse>(
	...
	async function handleUploadSubmit(event: FormEvent<HTMLFormElement>) {
		...
		try {
			...
			setNotice({
				kind: "success",
				text: "Upload accepted. The worker will process it from RabbitMQ.",
			});
		} catch (error) {
			setNotice({ kind: "error", text: errorMessage(error) });
		} finally {
		...
		<section className="grid gap-8 lg:grid-cols-[1fr_420px] lg:items-center">
			<div className="max-w-2xl">
				...
				<p className="mt-4 text-base leading-7 text-slate-600">
					Upload a mass spectrometry file, let the RabbitMQ worker run the
					m/z-only model, and view the final lipid class prediction with its
					probability score.
				</p>
			</div>
			...
			<div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
				<div className="mb-6">
					...
					<p className="mt-2 text-sm leading-6 text-slate-600">
						The backend stores the file, creates a job, and publishes the
						RabbitMQ message that the ML worker consumes.
					</p>
				</div>
...
function ResultScreen({
	...
	return (
		<section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
			...
			{!failed && job.top_predictions.length > 0 ? (
				<TopPredictions predictions={job.top_predictions} />
			) : null}
	...
	return (
		<footer className="border-t border-slate-200 bg-white">
			<div className="mx-auto max-w-6xl px-4 py-4 text-sm text-slate-500 sm:px-6 lg:px-8">
				m/z-only baseline classifier using Spring Boot, RabbitMQ, PostgreSQL,
				and a Python worker.
			</div>
		</footer>
	);
...
async function apiRequest<T>(
	...
	if (!response.ok) {
		...
		throw new Error(
			parseErrorText(text) || `Request failed with HTTP ${response.status}`,
		);
	}
...
function formatDateMK(isoString: string): string {
